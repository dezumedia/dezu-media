...
>(({ className, ...props }, ref) => (
  <SideBarPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-50 bg-black dark:bg-zinc-800 opacity-70  data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    {...props}
    ref={ref}
...
const SideBarVariants = cva(
  "fixed z-50 overflow-auto gap-4 bg-white px-4 py-3 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 dark:bg-zinc-950",
  {
    variants: {
      side: {
...
>(({ side = "left", className, children, ...props }, ref) => (
  <SideBarPortal>
    ...
    >
      {children}
      {/* <SideBarPrimitive.Close className="absolute right-2 top-4 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-zinc-100 dark:data-[state=open]:bg-zinc-800">
